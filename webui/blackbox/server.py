...
def api_log(
    ...
    try:
        placeholders = ",".join("?" for _ in SHIP_EVENTS)
        rows = conn.execute(
            f"SELECT timestamp, event, raw_json FROM events WHERE event IN ({placeholders}) ORDER BY timestamp DESC LIMIT ? OFFSET ?",
            (*SHIP_EVENTS, limit, offset),
        ).fetchall()
        entries = []

...
def _parse_localisation_key(key: str) -> str:
    ...
    # $Codex_Ent_<Genus>[_<Num>[_<Variant>]]_Name  → "Genus [Num [Variant]]"
    m = re.match(r"Codex_Ent_[A-Za-z]+(?:_\d+(?:_[A-Za-z]+)?)?_Name$", key)
    if m:
        body = key[len("Codex_Ent_"):-len("_Name")]
        return body.replace("_", " ")

    # Clean fallback for any other $Codex_* key
    key = re.sub(r"_(?:Name)?$", "", key)
